...
export default function App() {
  ...
  const triggerSampleScan = (category: string) => {
    setIsLoading(true);
    // Simulate loading sequentially to give a premium feel and let users experience the scans logs!
    setTimeout(() => {
  ...
  return (
    <div className="bg-[#040201] min-h-screen text-neutral-100 flex flex-col font-sans selection:bg-[#f97316]/30 selection:text-white" id="main-app-root">
      
      {/* GLOBAL BANNER IF GEMINI KEY ABSENT OR SANDBOX ACTIVE */}
      <div className="bg-gradient-to-r from-[#ea580c] to-[#f97316] text-black py-2 px-4 text-center text-[10.5px] font-mono tracking-wider font-bold flex items-center justify-center gap-2 select-none" id="production-environment-alert">
        <Sparkles className="w-3.5 h-3.5 text-white animate-spin" />
        <span>POWERED BY THE GEMINI COGNITIVE CORE • REAL-TIME GAP ANALYSIS INTERFACES ACTIVE</span>
      </div>
        ...
        {activeTab === "help" && (
          <div className="max-w-4xl mx-auto px-4 py-12 space-y-12" id="faq-help-screen">
            ...
            <div className="text-center space-y-3">
              <span className="px-3 py-1 bg-[#f97316]/10 text-[#f97316] border border-[#f97316]/25 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider">
                Support & Guides
              </span>
              <h1 className="text-3xl font-black font-display uppercase text-white tracking-tight sm:text-4xl">
            ...
            <div className="bg-[#0c0a09] border border-neutral-900 rounded-2xl p-6 shadow-xl space-y-4">
              <h2 className="text-lg font-bold font-display uppercase tracking-wider text-white mb-6 flex items-center gap-2">
                  ...
                  return (
                    <div 
                      ...
                      className={`border rounded-xl transition-all ${
                        isOpen 
                          ? "bg-[#040201] border-[#f97316]/30" 
                          : "bg-[#040201]/40 border-neutral-900 hover:border-neutral-800"
                      }`}
                      id={`faq-node-${faq.id}`}
                    >
                      <button
                        onClick={() => setOpenFaqId(isOpen ? null : faq.id)}
                        className="w-full text-left px-5 py-4 flex items-center justify-between gap-4 font-semibold text-sm text-white transition-colors hover:text-[#f97316] cursor-pointer"
                        type="button"
                      >
                        <span>{faq.q}</span>
                        <ChevronDown className={`w-4 h-4 text-neutral-500 flex-shrink-0 transition-transform duration-300 ${isOpen ? "rotate-180 text-[#f97316]" : ""}`} />
                      </button>

                      {isOpen && (
                        <div className="px-5 pb-5 pt-1 border-t border-neutral-900 text-xs sm:text-sm text-neutral-400 leading-relaxed font-sans">
                          {faq.a}
                        </div>
                      )}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="benchmark-recommendations">
              ...
              <div className="bg-gradient-to-br from-[#0c0a09] to-[#040201] border border-neutral-900 p-6 rounded-2xl space-y-3 shadow-xl">
                <div className="p-2 bg-[#f97316]/10 text-[#f97316] rounded-xl w-fit">
                  <Lightbulb className="w-5 h-5" />
                </div>
                <h3 className="font-bold text-base text-white uppercase font-display tracking-wider">Conversion rate boosters</h3>
                ...
                <div className="text-[10px] text-neutral-550 font-mono flex items-center gap-1.5 pt-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-[#f97316]" /> High Priority Impact
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-[#0c0a09] to-[#040201] border border-neutral-900 p-6 rounded-2xl space-y-3 shadow-xl">
                ...
                <h3 className="font-bold text-base text-white uppercase font-display tracking-wider">Web core speeds strategy</h3>
                <p className="text-xs text-neutral-400 leading-relaxed">
                ...
                <div className="text-[10px] text-neutral-550 font-mono flex items-center gap-1.5 pt-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-[#f97316]" /> Web Core Vitals Compliant
                </div>
              </div>
      ...
      {/* FOOTER */}
      <footer className="bg-[#040201] border-t border-neutral-900 py-8 text-center text-neutral-500 text-xs font-sans mt-auto" id="app-footer">
        <div className="max-w-7xl mx-auto px-4 space-y-4">
          <p className="font-mono text-[9px] tracking-widest uppercase text-neutral-500">
